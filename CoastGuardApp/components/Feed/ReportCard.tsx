        ...
        <TouchableOpacity onPress={onPress} activeOpacity={0.8} className="bg-gray-800 p-4 border-b border-gray-700">
            ...
            <View className="flex-row items-center mb-2">
                <View className="w-10 h-10 bg-gray-600 rounded-full items-center justify-center mr-3">
                    <Text className="text-white font-bold">{report.user_id?.substring(0, 1) || 'U'}</Text>
                </View>
                <View>
                    <Text className="text-white font-bold text-base">User {report.user_id?.substring(0, 4)}</Text>
                ...
                <View className={`ml-auto px-2 py-1 rounded-full ${report.status_name === 'official_verified' ? 'bg-green-900/50 border border-green-500' :
                    report.status_name === 'fake' ? 'bg-red-900/50 border border-red-500' : 'bg-gray-700'
                    }`}>
                    <Text className={`text-xs font-bold ${report.status_name === 'official_verified' ? 'text-green-400' :
                        report.status_name === 'fake' ? 'text-red-400' : 'text-gray-400'
                        }`}>
                        {report.status_name === 'official_verified' ? 'VERIFIED' : report.status_name || 'UNVERIFIED'}
                    </Text>
                </View>
            </View>
